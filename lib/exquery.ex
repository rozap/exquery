defmodule Exquery do
  ...
  def to_attributes(" "  <> r, :key, acc, attrs),         do: to_attributes(r, :key,     acc, attrs)
  ...
  def to_attributes(r, attrs), do: to_attributes(r, :key, {"", ""}, attrs)

  defp attributes(r, {tag, text, attrs} = token, acc) do
    {r, attrs} = to_attributes(r, :key, {"", ""}, attrs)
  ...
  defp ff(token, "<" <> r, acc) do
    {"<" <> r, push_token(token, acc)}
  end
